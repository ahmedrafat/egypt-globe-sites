  ...
  contact: [
    {
      ...
      answer: 'Every request is acknowledged by the export desk and answered with a priced offer once stock, laycan and freight are confirmed. Our export desk in Cairo handles inbound RFQs Sunday through Thursday (Egyptian working week) plus reduced cover Friday + Saturday for urgent requests.',
    },
    {
    ...
    },
    {
      ...
      answer: 'Every shipment ships with a Certificate of Analysis from our internal QC lab. Independent third-party pre-shipment inspection (TÜV Austria / SGS / Intertek / Bureau Veritas) is available on request — typical cost 0.3-0.5% of FOB value. TÜV Austria, SGS and Intertek are our primary inspection partners. Specifications are guaranteed at port of loading and binding under the Sales Contract.',
    },
    {
  ...
  default: [
    {
      ...
      answer: 'Use the RFQ form at /rfq, or email export@egyptglobe.com directly. The export desk replies to every request.',
    },
    {
 ...
 */
/** A name a buyer would type, not the SEO title: "Granular Sulphur Supplier in
 *  Egypt — 99.9% Bulk Export" made every question read like a headline. The
 *  commodity row keeps the plain product name; long titles fall back to the
 *  part before the dash. */
export function productName(page, commodity) {
  const title = String(page?.title || '').replace(/\s+Supplier( in Egypt)?\b/i, '').replace(/\s+—\s*$/, '').trim()
  const cname = String(commodity?.name || '').trim()
  if (cname && cname.length <= 60 && cname !== page?.title) return cname
  if (title.length <= 60) return title
  return title.split(' — ')[0].trim() || title
}

/** Top measurable limits from quality_specs, for SKUs whose `specs` jsonb has
 *  no NaCl / standard / strength field to build the purity question from. */
function specLine(qualitySpecs) {
  const rows = (qualitySpecs || []).filter(q =>
    q?.target_value && /\d/.test(String(q.target_value)) &&
    !/^(origin|colou?r|appearance|storage|shelf)/i.test(q.parameter_name || ''))
  // Purity / assay / grade limits lead — they are what the question is about;
  // then other required limits, then the rest.
  const lead = q => /purity|assay|content|\bmin\b|grade|strength|nacl|sulphur|p2o5|p₂o₅|k2o|k₂o|\bn\b|sio2|sio₂|caco3|fe\b/i.test(`${q.parameter_name} ${q.parameter_key || ''}`) ? 2 : 0
  rows.sort((a, b) => (lead(b) + (b.required ? 1 : 0)) - (lead(a) + (a.required ? 1 : 0)))
  return rows.slice(0, 4).map(q => {
    const tv = String(q.target_value)
    const unit = q.unit && !/\d\s*(%|°|[A-Za-zµ])/.test(tv) && !tv.includes(q.unit) ? ` ${q.unit}` : ''
    return `${q.parameter_name} ${tv}${unit}`
  })
}

export function generateProductFaqs(page, commodity, qualitySpecs) {
  if (!page) return []
  const out = []
  const name = productName(page, commodity)
  ...
  if (specs.nacl_min) {
    out.push({
      question: `What is the NaCl purity of ${name}?`,
      answer: `Minimum NaCl ${specs.nacl_min}, guaranteed per shipment via Certificate of Analysis from our internal QC lab. Independent third-party verification (TÜV Austria / SGS / Intertek / Bureau Veritas) available on request.`,
    })
  } else if (specs.standard) {
    out.push({
      question: `What standard does ${name} comply with?`,
      answer: `${name} is produced and verified to ${specs.standard}. Per-shipment Mill Test Certificate or Certificate of Analysis confirms compliance.`,
    })
  } else if (specs.compressive_28d) {
    out.push({
      question: `What is the 28-day compressive strength of ${name}?`,
    ...
    const lines = specLine(qualitySpecs)
    if (lines.length >= 2) {
      out.push({
        question: `What is the specification of ${name}?`,
        answer: `${lines.join('; ')}. Each limit is verified per lot and stated on the Certificate of Analysis issued before the Bill of Lading; independent inspection (TÜV Austria / SGS / Intertek / Bureau Veritas) is available on request.`,
      })
    }
  }
  ...
  if (packing.length > 0) {
    ...
    out.push({
      question: `What packing formats are available for ${name}?`,
  ...
  if (page.lead_time_min_weeks || page.lead_time_max_weeks) {
    const lead = page.lead_time_min_weeks && page.lead_time_max_weeks
      ...
      : `${page.lead_time_min_weeks || page.lead_time_max_weeks} weeks`
    out.push({
      question: `What is the lead time for ${name}?`,
  ...
  if (certs.length > 0) {
    ...
    out.push({
      question: `What certifications does ${name} carry?`,
 ...
 */
export function faqsForPage(page, commodity, qualitySpecs) {
  if (!page) return []
  // Page-specific FAQs from the CMS (egg_corporate_pages.seo.faqs) win over
  // the generic category sets — lets each commodity / market page target its
  // own People-Also-Ask questions with unique content instead of the shared
  // boilerplate every /markets/* page used to show.
  const custom = Array.isArray(page.seo?.faqs)
  ...
  const productFaqs = generateProductFaqs(page, commodity, qualitySpecs)
