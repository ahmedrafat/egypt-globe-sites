...
/**
 * Build 6-10 FAQs from a single egg_corporate_pages row + optional commodity.
 * Returns [] when the page has no useful structured data.
 */
export function generateProductFaqs(page, commodity) {
  ...
  // Q1 — purity / grade / standard (depends on what's available)
  if (specs.nacl_min) {
    out.push({
  ...
  if (ports.length > 0) {
    ...
    out.push({
      question: `Which Egyptian port do you load ${page.title} from?`,
      answer: `Loads from ${portList}. Choice of loading port depends on vessel size, destination, and source location. We optimise per shipment to minimise total freight cost.`,
    })
  }
  ...
  if (regions.length > 0) {
    ...
    const more = regions.length > 5 ? ` and ${regions.length - 5} more` : ''
    out.push({
      question: `Which markets do you ship ${page.title} to?`,
      answer: `Active destination markets include ${regionList}${more}. Buyer references in your industry / region available under NDA.`,
    })
  }
  ...
  if (page.hs_code) {
    out.push({
      question: `What is the HS code for ${page.title}?`,
      answer: `HS ${page.hs_code}. Note: destination-country customs may apply 8/10/12-digit national extensions. We provide line-item-level HS classification on the Commercial Invoice.`,
    })
  }

  // Q9 — origin (commodity-master enrichment if present)
  if (commodity?.origin || specs.origin) {
    const origin = specs.origin || commodity.origin
    out.push({
      question: `Where is ${page.title} sourced from?`,
      answer: `Sourced from ${origin}. Provenance is documented on the Certificate of Origin issued by the Egyptian Chamber of Commerce — qualifies for COMESA / PAFTA / EU-Med preferential treatment where applicable.`,
    })
  }

  // Q10 — Incoterms / pricing
  out.push({
    question: `What Incoterms do you quote for ${page.title}?`,
    answer: `FOB, CIF, CFR are standard. CPT, CIP, EXW, DAP, DDP, DPU, FCA, FAS available on request. Quotations remain valid 7-14 days FOB / 14-30 days CIF — re-quotable instantly if rates move.`,
  })
