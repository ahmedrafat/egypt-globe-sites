 ...
 * (currently `/`, `/rfq`).
 */
const RESERVED_PATHS = new Set(['/', '/rfq', '/products/salt'])
...
export async function getPagesForApplication(appId, { limit = 60 } = {}) {
  ...
  const { data, error } = await supabase
    .from('egg_corporate_pages')
    .select('id, path, title, hero_photo_url, description, category, hs_code, price_indication, packing_options, certifications')
    .eq('is_published', true)
...
export async function getSaltCatalogueBySource() {
  const { data, error } = await supabase
    .from('egg_corporate_pages')
    .select('id, path, title, hero_photo_url, description, hs_code, price_indication, packing_options, certifications, applications, specs, moq_mt')
    .eq('is_published', true)
    .eq('category', 'salt')
...
export async function getRfqProductOptions() {
  ...
  const { data, error } = await supabase
    .from('egg_corporate_pages')
    .select('id, path, title, category, hs_code, packing_options, moq_mt, specs, certifications, lead_time_min_weeks, lead_time_max_weeks, applications, regions, price_indication')
    .eq('is_published', true)
