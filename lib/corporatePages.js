...
export const CATEGORY_META = {
  home:         { label: 'Home',         icon: 'building', color: '#1d5fa1', tone: 'bg-blue-50  text-blue-700  border-blue-100'   },
  about:        { label: 'About',        icon: 'building', color: '#6366f1', tone: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
  products:     { label: 'Products',     icon: 'box', color: '#1d5fa1', tone: 'bg-blue-50  text-blue-700  border-blue-100'   },
  salt:         { label: 'Salt',         icon: 'cube', color: '#0ea5e9', tone: 'bg-sky-50    text-sky-700    border-sky-100'    },
  fertilizers:  { label: 'Fertilizers',  icon: 'wheat', color: '#16a34a', tone: 'bg-green-50  text-green-700  border-green-100'  },
  chemicals:    { label: 'Chemicals',    icon: 'beaker', color: '#db2777', tone: 'bg-pink-50   text-pink-700   border-pink-100'   },
  construction: { label: 'Construction', icon: 'building', color: '#d97706', tone: 'bg-amber-50  text-amber-700  border-amber-100'  },
  agro:         { label: 'Agro & Food',  icon: 'leaf', color: '#059669', tone: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
  minerals:     { label: 'Minerals',     icon: 'layers',  color: '#64748b', tone: 'bg-slate-50  text-slate-700  border-slate-200'    },
  metals:       { label: 'Metals & Alloys', icon: 'gear', color: '#71717a', tone: 'bg-zinc-50   text-zinc-700   border-zinc-200'    },
  services:     { label: 'Services',     icon: 'ship', color: '#0d9488', tone: 'bg-teal-50   text-teal-700   border-teal-100'   },
  applications: { label: 'Application',  icon: 'factory', color: '#7c3aed', tone: 'bg-violet-50 text-violet-700 border-violet-100' },
  case_studies:{ label: 'Case Study',    icon: 'book', color: '#0d9488', tone: 'bg-teal-50    text-teal-700    border-teal-100'   },
  markets:      { label: 'Market Intel',  icon: 'chart', color: '#0891b2', tone: 'bg-cyan-50   text-cyan-700   border-cyan-100'   },
  partners:     { label: 'Partners',     icon: 'handshake', color: '#2563eb', tone: 'bg-blue-50   text-blue-700   border-blue-100'   },
  blog:         { label: 'News & Blog',  icon: 'news', color: '#e11d48', tone: 'bg-rose-50   text-rose-700   border-rose-100'   },
  rfq:          { label: 'RFQ',          icon: 'doc', color: '#ea580c', tone: 'bg-orange-50 text-orange-700 border-orange-100' },
  // Categories that exist in egg_corporate_pages but had no entry here — they
  // silently fell back to `other`, so ~50 pages (every port, standard and
  // wholesale page) labelled themselves "More" in the chip, the breadcrumb
  // and the "More in …" heading. Real labels, real colours.
  ports:        { label: 'Ports',        icon: 'anchor', color: '#0284c7', tone: 'bg-sky-50    text-sky-700    border-sky-100'    },
  standards:    { label: 'Standards',    icon: 'shield', color: '#0d9488', tone: 'bg-teal-50   text-teal-700   border-teal-100'   },
  wholesale:    { label: 'Wholesale',    icon: 'box', color: '#1d5fa1', tone: 'bg-blue-50   text-blue-700   border-blue-100'   },
  trade_tools:  { label: 'Trade Tools',  icon: 'coins', color: '#b8862b', tone: 'bg-amber-50  text-amber-700  border-amber-100'  },
  other:        { label: 'Resources',    icon: 'grid', color: '#475569', tone: 'bg-slate-50  text-slate-700  border-slate-200'  },
}

/**
 * Some pages sit in the catch-all `other` category but clearly belong to a
 * named family by path. Resolving the label from the path keeps the chip,
 * breadcrumb and related-pages heading meaningful instead of "Resources".
 */
const PATH_CATEGORY = [
  [/^\/ports(\/|$)/,        'ports'],
  [/^\/standards(\/|$)/,    'standards'],
  [/^\/wholesale(\/|$)/,    'wholesale'],
  [/^\/trade-tools(\/|$)/,  'trade_tools'],
  [/^\/services(\/|$)/,     'services'],
  [/^\/markets(\/|$)/,      'markets'],
  [/^\/applications(\/|$)/, 'applications'],
  [/^\/(about|global-presence|investors|partners)(\/|$)/, 'about'],
]

/** Resolve display metadata for a page, preferring an explicit category. */
export function categoryMetaFor(category, path = '') {
  // A real, specific category always wins. `other` is a catch-all, not a
  // statement about the page, so a path match beats it.
  if (category && category !== 'other' && CATEGORY_META[category]) return CATEGORY_META[category]
  for (const [re, key] of PATH_CATEGORY) if (re.test(path || '')) return CATEGORY_META[key]
 ...
 *  `color` is the accent (dots, icons, rings, tints); `textColor` is the
 *  darker variant to use whenever the division colour becomes text — the
 *  accents sit at 2.5–4.4:1 on white, below the 4.5:1 AA threshold. */
export const PRODUCT_DIVISIONS = [
  { id: 'salt',         path: '/products/salt',         label: 'Salt',                  icon: 'cube', color: '#0ea5e9', textColor: '#0369a1', blurb: 'Sea & rock salt from food and pharma grades to de-icing and chlor-alkali.' },
  { id: 'fertilizers',  path: '/products/fertilizers',  label: 'Fertilizers',           icon: 'wheat', color: '#16a34a', textColor: '#15803d', blurb: 'Urea, MOP, TSP, DAP, MAP, NPK, SOP — nitrogen, phosphate and potassium grades.' },
  { id: 'chemicals',    path: '/products/chemicals',    label: 'Chemicals',             icon: 'beaker', color: '#db2777', textColor: '#be185d', blurb: 'Industrial chemicals, polymers, plastics, solvents, fuels and petroleum products.' },
  { id: 'construction', path: '/products/construction', label: 'Construction',          icon: 'building', color: '#d97706', textColor: '#b45309', blurb: 'Cement, clinker, aggregates, granite, marble, gypsum, limestone, glass, ceramics.' },
  { id: 'agro',         path: '/products/agro',         label: 'Agro & Food',           icon: 'leaf', color: '#059669', textColor: '#047857', blurb: 'Grains, edible oils, cotton, sugar, spices, fresh produce, seafood and animal feed.' },
  { id: 'minerals',     path: '/products/minerals',     label: 'Industrial Minerals',   icon: 'layers',  color: '#64748b', textColor: '#475569', blurb: 'Silica sand, kaolin, barite, feldspar, talc, bentonite, iron ore, coal — raw materials.' },
  { id: 'metals',       path: '/products/metals',       label: 'Metals & Alloys',       icon: 'gear', color: '#71717a', textColor: '#52525b', blurb: 'Steel, aluminum, copper, zinc, lead, ferro-alloys and scrap metal.' },
]
 ...
 */
/**
 * Long-tail application pages that are published but deliberately kept out of
 * APPLICATIONS, which drives the header nav and footer — adding them there
 * would bloat every menu on the site. The /applications hub still lists them
 * so all 29 published application pages are reachable from the picker.
 *
 * `divisions` mirrors the canonical parent's divisions so each card lands in
 * the right group; `icon` and `blurb` are the card's own.
 */
export const APPLICATION_VARIANTS = [
  { id: 'aquaculture',           path: '/applications/aquaculture',           label: 'Aquaculture',              icon: 'wave',
    blurb: 'Salinity control and fish-health salt for hatcheries and shrimp farms.',
    divisions: ['salt', 'agro'] },
  { id: 'chemical',              path: '/applications/chemical',              label: 'Chlor-Alkali & Chemical',  icon: 'beaker',
    blurb: 'Feedstock salt for chlorine, caustic soda, soda ash and PVC production.',
    divisions: ['chemicals', 'salt'] },
  { id: 'chlor-alkali-salt',     path: '/applications/chlor-alkali-salt',     label: 'Chlor-alkali — membrane grade', icon: 'beaker',
    blurb: 'Membrane-cell premium salt with tight calcium, magnesium and sulphate caps.',
    divisions: ['chemicals', 'salt'] },
  { id: 'deicing-rock-salt',     path: '/applications/deicing-rock-salt',     label: 'De-icing — rock salt',     icon: 'snow',
    blurb: 'Mined highway and motorway road salt, EN 16811-1 and ASTM D632 grades.',
    divisions: ['salt'] },
  { id: 'deicing-sea-salt',      path: '/applications/deicing-sea-salt',      label: 'De-icing — sea salt',      icon: 'snow',
    blurb: 'Solar-evaporated road salt for EU, UK, US and Nordic winter tenders.',
    divisions: ['salt'] },
  { id: 'road-salt',             path: '/applications/road-salt',             label: 'Road salt — wholesale',    icon: 'snow',
    blurb: 'Bulk wholesale road salt for highway authorities and winter stockpiles.',
    divisions: ['salt'] },
  { id: 'food_preservation',     path: '/applications/food_preservation',     label: 'Food Preservation',        icon: 'leaf',
    blurb: 'Curing, brining and smoking salt for meat, fish and dairy processors.',
    divisions: ['salt', 'agro'] },
  { id: 'table_salt',            path: '/applications/table_salt',            label: 'Table & Retail Salt',      icon: 'leaf',
    blurb: 'Iodised and fine table salt for retail packing and private label.',
    divisions: ['salt', 'agro'] },
  { id: 'leather-tanning-salt',  path: '/applications/leather-tanning-salt',  label: 'Leather tanning',          icon: 'factory',
    blurb: 'Hide preservation and curing salt for tanneries, LWG-compatible.',
    divisions: ['salt', 'chemicals'] },
  { id: 'textile-dyeing-salt',   path: '/applications/textile-dyeing-salt',   label: 'Textile dyeing',           icon: 'factory',
    blurb: 'Ultra-low-sulphate salt for reactive dye fixation in textile mills.',
    divisions: ['salt', 'chemicals', 'agro'] },
  { id: 'oil-gas-salt',          path: '/applications/oil-gas-salt',          label: 'Oil & gas — drilling salt', icon: 'ship',
    blurb: 'Drilling mud, workover and completion-fluid salt to API 13B-1.',
    divisions: ['salt', 'chemicals'] },
  { id: 'water-treatment-salt',  path: '/applications/water-treatment-salt',  label: 'Water treatment — salt',   icon: 'drop',
    blurb: 'Softener pellets and ion-exchange crystal for municipal and RO plants.',
    divisions: ['salt', 'chemicals'] },
  { id: 'pool',                  path: '/applications/pool',                  label: 'Swimming Pool',            icon: 'drop',
    blurb: 'Chlorinator-grade pool salt, no anti-caking agent, fast dissolving.',
    divisions: ['salt'] },
]

export const APPLICATIONS = [
  { id: 'construction',     path: '/applications/construction',     label: 'Construction & Infrastructure', icon: 'building',
  ...
  { id: 'stone_architecture', path: '/applications/stone_architecture', label: 'Stone & Architecture',        icon: 'layers',
  ...
  { id: 'food_processing',  path: '/applications/food_processing',  label: 'Food Processing & Manufacturing', icon: 'leaf',
  ...
  { id: 'retail_horeca',    path: '/applications/retail_horeca',    label: 'Retail & HoReCa',                 icon: 'box',
    blurb: 'Table salt, retail packs, hospitality, catering, gourmet — branded consumer formats with OEM print.',
    matches: ['table_salt', 'table_consumer', 'retail', 'retail_frozen', 'horeca', 'catering', 'gifting', 'wholesale_bulk', 'luxury_food', 'health_food', 'nutraceuticals', 'bakery', 'confectionery'],
    categories: [],
    divisions: ['salt', 'agro'] },
  { id: 'industrial_chemistry',  path: '/applications/industrial_chemistry', label: 'Industrial Chemistry',         icon: 'beaker',
  ...
  { id: 'water_treatment',  path: '/applications/water_treatment',  label: 'Water Treatment',                 icon: 'drop',
  ...
  { id: 'oil_gas',          path: '/applications/oil_gas',          label: 'Oil & Gas',                       icon: 'factory',
    blurb: 'Drilling muds, well cementing, refining, sulphur recovery — upstream + downstream oilfield services.',
    matches: ['oil_gas', 'oil_refinery', 'oilwell_cementing', 'drilling_fluid_weighting', 'sulphur_recovery', 'plug_abandonment', 'well_completion', 'squeeze_cementing', 'primary_cementing'],
    categories: [],
    divisions: ['salt', 'construction', 'chemicals', 'minerals', 'fertilizers'] },
  { id: 'agriculture',      path: '/applications/agricultural',     label: 'Agriculture',                     icon: 'wheat',
    blurb: 'Broadacre crops, hydroponics, soil amendment, fertigation — fertilizers, ag-lime, gypsum, sulphur.',
    matches: ['agriculture', 'agricultural', 'agricultural_lime', 'fertigation', 'foliar_spray', 'intensive_farming', 'precision_agriculture', 'specialty_crops', 'premium_agriculture', 'sulphur_deficient_soils', 'soil_acidification', 'npk_blending', 'grain_production', 'citrus', 'vegetables', 'broadacre', 'horticulture', 'hydroponics'],
    categories: ['fertilizers', 'agro'],
    divisions: ['fertilizers', 'agro', 'minerals', 'salt'] },  // salt has 3 agricultural SKUs
  { id: 'animal_feed',      path: '/applications/animal_feed',      label: 'Animal Feed & Livestock',         icon: 'cow',
  ...
  { id: 'steel_metal',      path: '/applications/steel_metal',      label: 'Steel & Metalworking',            icon: 'gear',
  ...
  { id: 'deicing',          path: '/applications/deicing',          label: 'De-icing & Road Salt',            icon: 'snow',
    blurb: 'Highway maintenance, airport runways, municipal roads — bulk + treated road salt for winter ops.',
    matches: ['deicing', 'road_salt', 'highway_maintenance'],
    categories: [],
    divisions: ['salt'] },
  { id: 'pharmaceutical',   path: '/applications/pharmaceutical',   label: 'Pharmaceutical',                  icon: 'pill',
    blurb: 'IV saline, dialysis, USP / EP / BP grade salts, dental, nutraceuticals — pharma-grade purity.',
    matches: ['pharmaceutical', 'nutraceuticals'],
    categories: [],
    divisions: ['salt', 'chemicals'] },
  { id: 'cosmetic',         path: '/applications/cosmetic',         label: 'Cosmetic & Spa',                  icon: 'sparkle',
    blurb: 'Bath salts, body scrubs, Dead-Sea blends, dental cosmetics — beauty & wellness formats.',
    matches: ['cosmetic'],
    categories: [],
    divisions: ['salt', 'minerals'] },
  { id: 'glass_ceramics',    path: '/applications/glass_ceramics',   label: 'Glass & Ceramics',                icon: 'grid',
  ...
  { id: 'pulp_textile',     path: '/applications/pulp_textile',     label: 'Pulp, Paper & Textile',           icon: 'scroll',
    ...
    categories: [],
    divisions: ['salt', 'chemicals', 'agro'] },
  { id: 'mining_other',     path: '/applications/mining_other',     label: 'Mining & Other Industrial',       icon: 'pickaxe',
...
export const SERVICE_DIVISIONS = [
  { id: 'logistics',     path: '/services/logistics',      label: 'Logistics & Freight', icon: 'ship', color: '#0ea5e9', blurb: 'Sea, road, multimodal — ex 7 Egyptian ports, full track-and-trace.' },
  { id: 'port-services', path: '/services/port-services',  label: 'Port Services',       icon: 'anchor', color: '#1d5fa1', blurb: 'Stevedoring, vessel agency, berth booking, customs liaison.' },
  { id: 'added-value',   path: '/services/added-value',    label: 'Added-Value Processing', icon: 'beaker', color: '#a855f7', blurb: 'Blending, screening, washing, kiln-drying — match any tender spec.' },
  { id: 'packing',       path: '/services/packing',        label: 'Packing',             icon: 'box', color: '#FF6321', blurb: '50 kg PP / 1 MT FIBC / palletised / bulk — buyer-spec marking.' },
  { id: 'distribution',  path: '/services/distribution',   label: 'Distribution & Warehousing', icon: 'factory', color: '#22c55e', blurb: 'Bonded warehousing, cross-docking, final-mile delivery worldwide.' },
  { id: 'inspection',    path: '/services/inspection',     label: 'Inspection & QC',     icon: 'beaker', color: '#16a34a', blurb: 'TÜV Austria / SGS / Intertek / BV pre-shipment inspection at any port.' },
  { id: 'documentation', path: '/services/documentation',  label: 'Trade Documentation', icon: 'doc', color: '#0d9488', blurb: 'Full L/C-bank set + EUR1, COO, Phytosanitary, B/L originals.' },
]
...
export async function getCaseStudies({ limit = 20, exclude = null } = {}) {
  let q = supabase
    ...
    // Tie-break on path, not updated_at: several case studies share a
    // sort_order, and an updated_at tie-break let any edit (even a new hero
    // image) promote a study onto the homepage's three slots.
    .order('sort_order')
    .order('path')
    .limit(limit)
...
/** Sitemap projection of getAllPages — path, category and updated_at only.
 *  The full select carries body_markdown for 500 rows (~3 MB); the sitemap
 *  needs 80 KB. Same filter and order so the two never disagree on membership. */
export async function getSitemapPages() {
  const { data, error } = await withTimeout(
    supabase
      .from('egg_corporate_pages')
      .select('id, path, category, updated_at')
      .eq('is_published', true)
      .order('category')
      .order('sort_order')
      .order('title'),
...
/** Every published page WITHOUT `body_markdown`.
 *
 *  Same row shape as getAllPages minus the one heavy column. The nav menus
 *  render `path` and `title`; card grids add `description` and
 *  `hero_photo_url`. None of them show the body, but including it meant the
 *  header pulled ~3 MB of markdown from Supabase on every request and React
 *  serialised whatever reached a client component into the HTML — a page as
 *  small as /contact was shipping the full 34 KB body of
 *  /about/quality-compliance. Use getAllPages only where the body is
 *  genuinely needed (app/llms-full.txt). */
export async function getAllPagesLite() {
  const { data, error } = await withTimeout(
    supabase
      .from('egg_corporate_pages')
      .select('id, path, title, category, description, hero_photo_url, sort_order, last_crawled, hs_code, moq_mt, price_indication, updated_at')
      .eq('is_published', true)
      .order('category')
      .order('sort_order')
      .order('title'),
...
export async function getPagesByCategory() {
  const all = await getAllPagesLite()
  const grouped = {}
  for (const p of all) {
 ...
 *
 *  This used to be `.limit(limit)` with no `.order()`, so Postgres returned
 *  rows in heap order — which moves whenever any row is UPDATEd. Every CMS
 *  edit, even an image-only one, silently reshuffled the related links of
 *  every other page in the category. The whole category is now fetched in a
 *  fixed order and ranked by rankRelatedPages(), so the result only changes
 *  when a page is added, removed, re-pathed or re-sorted. */
export async function getRelatedPages(current, limit = 6) {
  if (!current?.category) return []
  const { data, error } = await withTimeout(
    supabase
      .from('egg_corporate_pages')
      .select('id, path, title, hero_photo_url, description, sort_order, specs')
      .eq('is_published', true)
  ...
  if (error) return []
  return rankRelatedPages(current, data || [], limit)
}

/** Deterministic related-page ranking.
 *
 *  1. Exact siblings (same parent path) first, then pages by how many parent
 *     segments they share with `current` — an import guide links to other
 *     import guides before /contact, a SKU to its own sub-category first.
 *  2. Within each tier, pages are ordered by (sort_order, path) and rotated
 *     to start just after `current`, so each page links to the next N in the
 *     ring. A fixed top-N would point every page at the same four and give
 *     the rest of the category no inbound related links; the ring spreads
 *     them evenly. */
export function rankRelatedPages(current, rows, limit = 6) {
  const segs = p => String(p || '').split('/').filter(Boolean)
  const parent = segs(current.path).slice(0, -1)
  const parentKey = parent.join('/')
  const tierOf = r => {
    const rp = segs(r.path).slice(0, -1)
    if (rp.join('/') === parentKey) return Infinity
    let i = 0
    while (i < rp.length && i < parent.length && rp[i] === parent[i]) i++
    return i
  }
  const cmp = (a, b) =>
    ((a.sort_order ?? 0) - (b.sort_order ?? 0)) || (a.path < b.path ? -1 : a.path > b.path ? 1 : 0)

  const tiers = new Map()
  for (const r of rows) {
    if (r.id === current.id || r.path === current.path) continue
    const t = tierOf(r)
  ...
  for (const t of [...tiers.keys()].sort((a, b) => b - a)) {
    const list = tiers.get(t).sort(cmp)
    const start = list.findIndex(r => cmp(r, current) > 0)
    const s = start === -1 ? 0 : start
    out.push(...list.slice(s), ...list.slice(0, s))
...
export async function getDirectChildren(parentPath) {
  ...
  const { data, error } = await withTimeout(
    supabase
      .from('egg_corporate_pages')
      .select('id, path, title, hero_photo_url, description, category, hs_code, price_indication, loading_ports, specs, applications')
      .eq('is_published', true)
      .like('path', norm + '/%')
      .order('sort_order')
      .order('path'),
...
export async function getDivisionSubcategories(divisionPath) {
  ...
  const { data, error } = await withTimeout(
    supabase
      ...
      // Ordered: sub-category landings slice the first 8 siblings, so heap
      // order here decided which sub-categories they linked to.
      .order('sort_order')
      .order('path'),
    4000,
    [],
...
/**
 * Commodity facts behind the SKUs under a division or sub-category path —
 * origin, HS, MOQ, lead time, packing, ports, regions, certifications,
 * applications. Feeds the "at a glance" tables (lib/productLines.js).
 * Callers intersect with the published pages they already hold.
 */
export async function getCommodityFacts(prefix) {
  if (!prefix) return []
  const norm = '/' + prefix.replace(/^\/+|\/+$/g, '')
  const { data, error } = await withTimeout(
    supabase
      .from('commodities')
      .select('page_path, name, grade, hs_code, origin, loading_ports, packing_options, moq_mt, lead_time_min_weeks, lead_time_max_weeks, regions, certifications, applications')
      .like('page_path', norm + '/%')
      .order('page_path')
      .limit(400),
    4000,
...
export async function getRfqProductOptions() {
  ...
  // Batch 2 — only the fields RFQForm reads; every column here is
  // serialised into the /rfq HTML for all ~290 SKUs.
  const { data, error } = await supabase
    .from('egg_corporate_pages')
    .select('id, path, title, category, hs_code, packing_options, moq_mt, certifications, lead_time_min_weeks, lead_time_max_weeks, applications, price_indication')
    .eq('is_published', true)
    .in('category', PRODUCT_CATS)
    .order('category')
    .order('title')
...
/**
 * Batch 2 — titles for a handful of ancestor paths (breadcrumb labels).
 * One small query; returns { path: title }.
 */
export async function getPageTitles(paths = []) {
  ...
  return Object.fromEntries(data.map(r => [r.path, r.title]))
}


/**
 * Batch 2 — hero image for a page. Rows that never had a hero set
 * (markets, ports, standards, wholesale …) fall back to the generated
 * textless background at /heroes/<slug>.png, which the generator writes
 * for every published page, so cards and heroes stay consistent without
 * a CMS write. Real photographs (any other URL) always win.
 */
/**
 * Card image for a page in a listing grid. Topic banners (/banners/topics/*)
 * are shared by many detail pages -- six de-icing pages carry the same one --
 * so in a grid they would repeat side by side. Cards for those pages fall back
 * to the page's own generated background; the hero still shows the banner.
 */
export function cardUrl(page) {
  if (page?.hero_photo_url?.startsWith('/banners/topics/')) {
    return heroUrl({ ...page, hero_photo_url: null })
  }
  return heroUrl(page)
}

export function heroUrl(page) {
  if (!page) return null
  if (page.hero_photo_url) return page.hero_photo_url
  if (!page.path) return null
  const slug = page.path.replace(/^\//, '').replace(/\//g, '-').replace(/[^a-z0-9-]/gi, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || 'home'
