/**
 * coaPrint.js — Drop 155. Render a stored Certificate of Analysis
 ...
 * Buyer / QC team can both produce a print-ready letterhead with
 * the actual measured results, lab name, dates, pass/fail verdict,
 * cargo + packing metadata, and signature blocks.
 *
 ...
 *                          commodityOrigin, commodityGrade, buyer }
 */
...
<div id="coa-print-doc" style="width:794px;font-family:Inter,Arial,sans-serif;color:${INK};padding:32px 40px;background:white;box-sizing:border-box">
  <!-- Letterhead -->
  <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:4px solid ${PRIMARY};padding-bottom:14px;margin-bottom:18px">
    <div>
      <div style="display:flex;align-items:center;gap:10px">
        <div style="width:46px;height:46px;background:${ORANGE};border-radius:10px;color:white;font-weight:900;font-size:20px;display:flex;align-items:center;justify-content:center">EG</div>
        <div>
          <div style="font-weight:900;font-size:16px;letter-spacing:1.2px">EGYPT GLOBE GROUP</div>
          <div style="font-size:10px;color:#64748b;letter-spacing:1px">B2B EXPORT TRADING CONGLOMERATE</div>
        </div>
      </div>
    </div>
    <div style="text-align:right;font-size:10.5px;color:#64748b;line-height:1.5">
      Cairo, Egypt · Damietta operations<br>
      export@egyptglobe.com · www.egyptglobe.com<br>
      Commercial Registry 73418 · Tax 655-527-427 · Export Lic 600010794
    </div>
  </div>

  <!-- Title + verdict -->
  <div style="display:flex;align-items:stretch;gap:12px;margin-bottom:14px">
    <div style="flex:1;background:linear-gradient(90deg,${PRIMARY},#14467a);color:white;padding:14px 18px;border-radius:8px">
      <div style="font-size:20px;font-weight:900;letter-spacing:1px">CERTIFICATE OF ANALYSIS</div>
      <div style="font-size:11px;opacity:0.85;margin-top:2px">Issued by ${escape(coa?.lab_name || 'Egypt Globe QC')}${coa?.lab_certificate_no ? ' · Cert ' + escape(coa.lab_certificate_no) : ''}</div>
    </div>
    ...
    <tr>
      <td colspan="2" style="padding:8px;border:1px solid ${BORDER};border-top:none;background:#fff7ed">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:18px">
          <div style="flex:1">
            <div style="font-size:9px;font-weight:700;color:${ORANGE};text-transform:uppercase;letter-spacing:0.6px">Product</div>
    ...
    <tr>
      <td style="width:50%;padding:8px;vertical-align:bottom">
        <div style="border-top:1px solid ${INK};padding-top:5px">
          <div style="font-weight:700;font-size:11px">QC Manager</div>
          <div style="font-size:9.5px;color:#64748b">${escape(coa?.lab_name || 'Egypt Globe Group · Damietta')}</div>
          <div style="margin-top:14px;font-size:9px;color:#94a3b8">Stamp / Signature / Date</div>
        </div>
      </td>
      <td style="width:50%;padding:8px;vertical-align:bottom">
        <div style="border-top:1px solid ${INK};padding-top:5px">
          <div style="font-weight:700;font-size:11px">Authorising Officer</div>
          <div style="font-size:9.5px;color:#64748b">Egypt Globe Group · Cairo head office</div>
          <div style="margin-top:14px;font-size:9px;color:#94a3b8">Stamp / Signature / Date</div>
        </div>
      </td>
    </tr>
  ...
  <div style="border-top:1px solid ${BORDER};margin-top:24px;padding-top:8px;font-size:8.5px;color:#94a3b8;text-align:center;line-height:1.5">
    This Certificate of Analysis is issued by Egypt Globe Group based on test results from
    ${escape(coa?.lab_name || 'an authorised QC laboratory')}.
    Specifications are guaranteed at the port of loading.
    Document reference: ${escape(coa?.ref_code || '—')}
  </div>
</div>`
}

/**
 * Open a popup window with the CoA HTML and trigger the browser's
 * print dialog. The user can then choose:
 *   - Send to a paper printer (for an L/C bank packet)
 *   - "Save as PDF" (every modern browser offers this in the print dialog)
 *
 * Returns true if the print window was successfully opened, false if a
...
export function printCoa(coa, opts = {}) {
  ...
  const title = `${refCode} — Egypt Globe Group`
  win.document.open()
  ...
  <style>
    ...
    @media screen {
      ...
      #print-toolbar button {
        background: #1d5fa1; color: white; border: 0; font-weight: 700;
        font-size: 13px; padding: 10px 16px; border-radius: 8px;
        cursor: pointer; box-shadow: 0 2px 8px rgba(29,95,161,0.3);
      }
      #print-toolbar button:hover { background: #14467a; }
      #print-toolbar .secondary { background: white; color: #1d5fa1; border: 1px solid #cbd5e1; }
...
<body>
  ...
  ${html}
  <script>
    // Auto-trigger the print dialog after the document is fully laid out
