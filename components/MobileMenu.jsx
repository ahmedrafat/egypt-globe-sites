...
function Chevron({ open }) {
  return (
    <svg
      className={`w-4 h-4 text-[#67707f] transition-transform duration-300 motion-reduce:transition-none ${open ? 'rotate-180' : ''}`}
...
function Eyebrow({ children }) {
  return (
    <p className="px-6 pt-6 pb-2 text-[11px] font-mono uppercase tracking-[0.2em] text-[#67707f] select-none">
      {children}
    </p>
  )
}
          ...
          <div className="flex items-center justify-between pl-6 pr-3 h-14">
            <Link href="/" onClick={close} className="flex items-center gap-2.5 min-w-0">
              <div className="w-7 h-7 rounded-lg bg-[#1d5fa1] flex items-center justify-center flex-shrink-0">
                <span className="text-white font-black text-[11px] leading-none">EG</span>
              </div>
              <div className="min-w-0">
                <div className="font-bold text-[13px] text-[#14161a] leading-tight truncate">Egypt Globe Group</div>
                <div className="text-[9px] font-mono uppercase tracking-[0.18em] text-[#67707f] leading-tight">Est. 2014 · Cairo</div>
              </div>
            </Link>
            <button
              type="button"
              ...
              className="flex items-center justify-center w-10 h-10 rounded-lg text-[#5b6577] hover:bg-[#f3f4f6] active:bg-[#e5e7eb] transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            <AccordionBody open={section === 'products'}>
              <div className="pb-3">
                <Link href="/products" onClick={close}
                  className="flex items-center justify-between mx-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-[#087a70] hover:bg-[#f9fafb]">
                ...
                {productDivisions.map(div => (
                  <Link key={div.id} href={div.path} onClick={close}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-[#67707f] truncate">{div.blurb}</div>
                    </div>
                  </Link>
                ))}
              </div>
                ...
                {serviceDivisions.map(svc => (
                  <Link key={svc.id} href={svc.path} onClick={close}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-[#67707f] truncate">{svc.blurb}</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="border-b border-[#14161a]/10">
            ...
            >
              <span className="flex items-baseline gap-2 text-[15px] font-semibold text-[#14161a]">
                Resources
                <span className="text-[9px] font-mono uppercase tracking-wider text-[#c2410c]">48 pages</span>
              </span>
            ...
            <AccordionBody open={section === 'resources'}>
              <div className="pb-4">
                {RESOURCE_GROUPS.map(group => (
                  <div key={group.label}>
                    <p className="px-6 pt-4 pb-1 text-[9px] font-mono uppercase tracking-[0.18em] text-[#67707f] select-none">
                      {group.label}
                    </p>
                    {group.links.map(([href, label]) => (
                      <Link key={href} href={href} onClick={close}
                        className="block mx-3 px-3 py-2 rounded-lg text-[13.5px] text-[#3f4650] hover:bg-[#f9fafb] hover:text-[#087a70] active:bg-[#f3f4f6] transition-colors">
                        {label}
                      </Link>
                    ))}
                  </div>
                ...
                <Link href="/search" onClick={close}
                  className="flex items-center gap-2 mx-3 mt-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-[#087a70] hover:bg-[#f9fafb]">
          ...
          <div className="pb-2">
            {COMPANY_LINKS.map(([href, label]) => (
              <Link key={href} href={href} onClick={close}
                ...
                {label}
                <svg className="w-3.5 h-3.5 text-[#67707f]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
            ...
            {visibility?.authenticated ? (
              <Link href="/buyer" onClick={close} className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-[#67707f] truncate">{visibility.email}</div>
                </div>
              </Link>
            ) : (
              <Link href="/login" onClick={close}
                className="flex items-center gap-2 text-[13.5px] text-[#5b6577] hover:text-[#14161a] transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
