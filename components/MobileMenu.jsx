...
function Chevron({ open }) {
  return (
    <svg
      className={`w-4 h-4 text-slate-400 transition-transform duration-300 motion-reduce:transition-none ${open ? 'rotate-180' : ''}`}
...
function Eyebrow({ children }) {
  return (
    <p className="px-6 pt-6 pb-2 text-[10px] font-mono uppercase tracking-[0.2em] text-slate-400 select-none">
      {children}
    </p>
  )
}
  ...
  return (
    <>
      ...
      <div
        onClick={close}
        aria-hidden="true"
        className={`lg:hidden fixed inset-0 z-50 bg-slate-900/45 backdrop-blur-[2px] transition-opacity duration-300 motion-reduce:transition-none ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      />
      ...
      <div
        ...
        aria-modal="true"
        tabIndex={-1}
        className="lg:hidden fixed inset-y-0 right-0 z-50 w-[min(20rem,88vw)] bg-white shadow-2xl shadow-slate-900/20 flex flex-col outline-none transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:transition-none"
      ...
      >
          ...
          <div className="flex items-center justify-between pl-6 pr-3 h-14">
            <Link href="/" onClick={close} className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-[13px] text-slate-900 leading-tight truncate">Egypt Globe Group</div>
                <div className="text-[9px] font-mono uppercase tracking-[0.18em] text-slate-400 leading-tight">Est. 2014 · Cairo</div>
              </div>
            </Link>
            <button
              type="button"
              ...
              className="flex items-center justify-center w-10 h-10 rounded-lg text-slate-500 hover:bg-slate-100 active:bg-slate-200 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <div key={openCount} className="flex-1 overflow-y-auto overscroll-contain stagger-children motion-reduce:[&>*]:!animate-none">
            ...
            <AccordionBody open={section === 'products'}>
              <div className="pb-3">
                <Link href="/products" onClick={close}
                  className="flex items-center justify-between mx-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-[#1d5fa1] hover:bg-slate-50">
                  All products <span aria-hidden="true">→</span>
                </Link>
                {productDivisions.map(div => (
                  <Link key={div.id} href={div.path} onClick={close}
                    className="flex items-center gap-3 mx-3 px-3 py-2.5 rounded-lg hover:bg-slate-50 active:bg-slate-100 transition-colors">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm flex-shrink-0"
                      style={{ background: `${div.color}15` }}>
                      {div.icon}
                    </div>
                    <div className="min-w-0">
                      <div className="font-medium text-[13.5px] text-slate-800 leading-tight">{div.label}</div>
                      <div className="text-[11px] text-slate-400 truncate">{div.blurb}</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="border-b border-slate-100">
            <button
              ...
              className="w-full flex items-center justify-between px-6 py-3.5 text-left"
            >
              <span className="text-[15px] font-semibold text-slate-900">Services</span>
              ...
              <div className="pb-3">
                ...
                {serviceDivisions.map(svc => (
                  <Link key={svc.id} href={svc.path} onClick={close}
                    ...
                    <div className="min-w-0">
                      <div className="font-medium text-[13.5px] text-slate-800 leading-tight">{svc.label}</div>
                      <div className="text-[11px] text-slate-400 truncate">{svc.blurb}</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="border-b border-slate-100">
            <button
              ...
              className="w-full flex items-center justify-between px-6 py-3.5 text-left"
            >
              <span className="flex items-baseline gap-2 text-[15px] font-semibold text-slate-900">
                Resources
            ...
            <AccordionBody open={section === 'resources'}>
              <div className="pb-4">
                {RESOURCE_GROUPS.map(group => (
                  <div key={group.label}>
                    <p className="px-6 pt-4 pb-1 text-[9px] font-mono uppercase tracking-[0.18em] text-slate-400 select-none">
                      {group.label}
                    </p>
                    {group.links.map(([href, label]) => (
                      <Link key={href} href={href} onClick={close}
                        className="block mx-3 px-3 py-2 rounded-lg text-[13.5px] text-slate-700 hover:bg-slate-50 hover:text-[#1d5fa1] active:bg-slate-100 transition-colors">
                        {label}
                      </Link>
                    ))}
                  </div>
                ...
                <Link href="/search" onClick={close}
                  className="flex items-center gap-2 mx-3 mt-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-[#1d5fa1] hover:bg-slate-50">
          ...
          <div className="pb-2">
            {COMPANY_LINKS.map(([href, label]) => (
              <Link key={href} href={href} onClick={close}
                className="flex items-center justify-between px-6 py-3 text-[14px] font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 active:bg-slate-100 transition-colors">
                {label}
                <svg className="w-3.5 h-3.5 text-slate-300" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
          ...
          {/* Account */}
          <div className="px-6 py-4 border-t border-slate-100 mb-2">
            {visibility?.authenticated ? (
              <Link href="/buyer" onClick={close} className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="font-semibold text-[13.5px] text-slate-900">My catalogue</div>
                  <div className="text-[11px] text-slate-400 truncate">{visibility.email}</div>
                </div>
              </Link>
            ) : (
        ...
        {/* Sticky footer CTA */}
        <div className="flex-shrink-0 border-t border-slate-100 p-4 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
          <Link
            href="/rfq"
            onClick={close}
            className="flex items-center justify-center gap-2 w-full bg-[#FF6321] hover:bg-[#e0541b] active:bg-[#c84512] text-white font-bold text-[15px] h-12 rounded-lg shadow-md shadow-orange-500/25 transition-colors mb-2.5"
          >
          ...
          <div className={`grid gap-2 ${waNumber ? 'grid-cols-3' : 'grid-cols-2'}`}>
            {s.phoneE164 && (
              <a href={`tel:${s.phoneE164}`}
                className="flex items-center justify-center gap-1.5 h-10 rounded-lg border border-slate-200 text-slate-600 font-medium text-[13px] hover:bg-slate-50 active:bg-slate-100 transition-colors">
