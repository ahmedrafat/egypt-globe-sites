...
export function ServiceJsonLd({ name, description, url }) {
  const json = {
    ...
    description: description || 'Request a FOB / CIF / CFR price quote for Egyptian salt, cement, fertilizers, chemicals, minerals, or agro commodities. Priced offers from the export desk.',
  ...
  const offers = page.price_indication && visibility?.showPrices
    ...
    : {
        ...
        priceSpecification: {
          ...
          priceCurrency: 'USD',
          valueAddedTaxIncluded: false,
          description: 'Price on request from the export desk. FOB / CIF / CFR from Egyptian ports.',
        },
        availability: 'https://schema.org/InStock',
        seller: { '@id': `${BASE}#org` },
        ...(page.moq_mt ? {
