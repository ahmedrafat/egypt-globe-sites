 ...
 *
 * White-on-deep-blue. Brand block + product columns + services column +
 * contact column + Egyptian commercial registry / tax / export-license
 * compliance strip. `settings` prop merges site_settings overrides over
 * the static default.
 */
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            <p className="mt-4 text-sm leading-relaxed text-slate-400 max-w-sm">
              {s.tagline}. Salt, cement, fertilizers, chemicals, construction
              materials, agro & food, industrial minerals — shipped from 7
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-2 text-sm">
              {SERVICE_DIVISIONS.map(svc => (
                <li key={svc.id}>
                  <Link href={svc.path} className="text-slate-400 hover:text-white transition-colors flex items-center gap-2">
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-3 text-sm">
              <li>
                ...
                <p className="text-slate-400 text-xs leading-relaxed">{s.headOffice}</p>
              </li>
              <li>
                <div className="font-semibold text-white text-xs mb-1">Operations — Damietta</div>
                <p className="text-slate-400 text-xs leading-relaxed">{s.operationsOffice}</p>
              </li>
              <li className="pt-1 border-t border-white/10">
                <a href={`tel:${s.phoneE164}`} className="block text-slate-300 hover:text-white">
                  📞 {s.phone}
                </a>
                <span className="block text-slate-500 text-xs mt-1">Tel & Fax: {s.telFax}</span>
                <a href={`mailto:${s.email}`} className="block text-slate-300 hover:text-white mt-1">
                  ✉ {s.email}
                </a>
              </li>
              <li>
        ...
        <div className="mt-12 pt-6 border-t border-white/10 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div className="rounded-lg bg-white/5 px-4 py-3">
            ...
            <div className="text-white font-semibold mt-0.5">{s.commercialRegistry}</div>
          </div>
          <div className="rounded-lg bg-white/5 px-4 py-3">
            <div className="text-slate-500 uppercase tracking-wider text-[10px]">Tax Card</div>
            <div className="text-white font-semibold mt-0.5">{s.taxCard}</div>
          </div>
          <div className="rounded-lg bg-white/5 px-4 py-3">
            <div className="text-slate-500 uppercase tracking-wider text-[10px]">Egyptian Export License</div>
            <div className="text-white font-semibold mt-0.5">{s.exportLicense}</div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 bg-black/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs">
          ...
          <div className="flex items-center gap-5 text-slate-500">
            ...
            {s.linkedin && (
              <a href={s.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-white">
                LinkedIn
              </a>
            )}
          </div>
