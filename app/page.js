...
export default async function HomePage() {
  ...
  return (
    <main className="egg-lux bg-white text-[#14110c] min-h-screen antialiased">
      {/* scoped styles — light editorial: warm hairlines, marquee, staggered reveal */}
      <style>{`
        .egg-lux{--line:rgba(184,146,90,.28)}
        .egg-grain::before{content:"";position:absolute;inset:0;pointer-events:none;opacity:.6;
          background-image:radial-gradient(rgba(20,17,12,.035) 1px,transparent 1px);background-size:3px 3px;z-index:0}
        .egg-vignette::after{content:"";position:absolute;inset:0;pointer-events:none;z-index:0;
          background:radial-gradient(120% 90% at 82% 0%,rgba(255,99,33,.06),transparent 55%),
                     radial-gradient(90% 70% at 4% 100%,rgba(184,146,90,.08),transparent 60%)}
      ...
      <section className="egg-grain egg-vignette relative min-h-[92vh] flex flex-col justify-between px-5 sm:px-10 lg:px-16 pt-20 sm:pt-24 pb-12 overflow-hidden">
        ...
        {/* light scrim keeps the photo a subtle backdrop; ink text stays readable */}
        {hasPhotos && <div className="absolute inset-0 z-[1] bg-gradient-to-b from-white/92 via-white/78 to-white/95" />}

        {/* overline */}
        <p className="egg-rise relative z-10 text-[10px] sm:text-[11px] font-mono uppercase tracking-[0.32em] text-[#a67c42]" style={{ animationDelay: '.05s' }}>
          Egypt Globe Group
          <span className="text-[#cbb48f] mx-2">/</span>
          <span className="text-[#8a8477]">Est. 2014 · Cairo · Damietta · 60+ markets</span>
        </p>

        {/* headline */}
        <div className="relative z-10 max-w-5xl w-full">
          <h1 className={`${display.className} egg-rise text-[clamp(2.6rem,8vw,7rem)] font-normal leading-[0.98] tracking-[-0.02em] text-[#14110c]`} style={{ animationDelay: '.12s' }}>
            Egypt&rsquo;s industrial<br />
            <span className="italic text-[#2a2418]">export operator</span><span className="text-[#ff6321]">.</span>
          </h1>
          <p className="egg-rise relative z-10 mt-7 sm:mt-9 text-base sm:text-lg lg:text-[1.35rem] max-w-2xl leading-relaxed text-[#5f5849]" style={{ animationDelay: '.2s' }}>
          ...
          <div className="egg-rise flex flex-wrap items-center gap-6 sm:gap-9 mt-9 sm:mt-11" style={{ animationDelay: '.28s' }}>
            <Link href="/rfq" className="group relative inline-flex items-center gap-2.5 text-sm font-semibold text-white bg-[#ff6321] px-7 py-3.5 rounded-full overflow-hidden transition-transform duration-300 hover:scale-[1.03] shadow-[0_8px_30px_-8px_rgba(255,99,33,.5)]">
              <span className="relative z-10">Request a quote</span>
              <span className="relative z-10 transition-transform duration-300 group-hover:translate-x-1">→</span>
              <span className="absolute inset-0 bg-[#14110c] translate-y-full group-hover:translate-y-0 transition-transform duration-400" />
            </Link>
            <Link href="/products" className="group inline-flex items-center gap-2 text-sm text-[#4a4437] hover:text-[#14110c] transition-colors">
        ...
        {/* certifications */}
        <div className="egg-rise relative z-10 flex flex-wrap gap-x-5 gap-y-1.5 text-[10px] sm:text-[11px] font-mono tracking-[0.14em] text-[#9c9585]" style={{ animationDelay: '.4s' }}>
          {['ISO 22000', 'EN 197-1', 'HACCP', 'USP / BP', 'GOEIC', 'TÜV AUSTRIA', 'SGS', 'INTERTEK'].map(c => (
            <span key={c} className="whitespace-nowrap hover:text-[#a67c42] transition-colors">{c}</span>
          ))}
        </div>
      </section>

      {/* ── 2. TICKER ───────────────────────────────────────────────── */}
      <div className="relative border-y border-[var(--line)] bg-[#faf8f3] overflow-hidden">
        <div className="egg-marquee py-3.5" aria-hidden="true">
          {[0, 1].map(dup => (
            <div key={dup} className="flex shrink-0">
              {TICKER.map((t, i) => (
                <span key={`${dup}-${i}`} className="flex items-center text-[11px] font-mono tracking-[0.2em] text-[#8a8477] px-6">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-[#faf8f3] to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-[#faf8f3] to-transparent" />
      </div>
      ...
      {heroBody && (
        <section className="border-b border-[var(--line)]">
          <div className="max-w-4xl mx-auto px-6 sm:px-10 lg:px-16 py-16">
            <article className="prose prose-slate max-w-none prose-headings:font-normal">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4">
          {METRICS.map((m, i) => (
            <div key={m.label} className={`px-8 sm:px-10 py-12 sm:py-16 ${i > 0 ? 'border-l border-[var(--line)]' : ''} ${i >= 2 ? 'border-t md:border-t-0 border-[var(--line)]' : ''} ${i === 2 ? 'border-l-0 md:border-l' : ''}`}>
              ...
              <p className="text-[11px] font-mono uppercase tracking-[0.2em] text-[#9c9585] mt-4">{m.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-b border-[var(--line)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3">
          {PILLARS.map((p, i) => (
            <div key={p.tag} className={`group px-8 sm:px-10 lg:px-14 py-14 border-t border-[var(--line)] ${i > 0 ? 'md:border-l md:border-t-0' : ''} hover:bg-[#faf8f3] transition-colors duration-500`}>
              <div className="flex items-baseline gap-4 mb-7">
                <span className={`${display.className} text-2xl text-[#d8cdb6] group-hover:text-[#a67c42] transition-colors duration-500`}>{p.n}</span>
                <p className="text-[11px] font-mono uppercase tracking-[0.22em] text-[#ff6321]">{p.tag}</p>
              </div>
              <p className="text-[15px] text-[#5f5849] leading-relaxed mb-9">{p.body}</p>
              <Link href={p.href} className="group/l inline-flex items-center gap-2 text-sm text-[#78715f] hover:text-[#14110c] transition-colors">
      ...
      <section className="border-b border-[var(--line)]">
        <div className="flex items-center justify-between px-6 sm:px-10 lg:px-16 pt-12 pb-4">
          <p className="text-[11px] font-mono uppercase tracking-[0.28em] text-[#9c9585]">Commodity divisions</p>
          <Link href="/products" className="group inline-flex items-center gap-1.5 text-[11px] font-mono uppercase tracking-[0.18em] text-[#78715f] hover:text-[#a67c42] transition-colors">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7">
          {PRODUCT_DIVISIONS.map((div, i) => (
            <Link key={div.id} href={div.path}
              className={[
                'group relative flex flex-col items-start gap-4 px-6 py-10 border-t border-[var(--line)] hover:bg-[#faf8f3] transition-colors duration-400',
                i % 2 !== 0 ? 'border-l border-[var(--line)]' : '',
                i % 4 === 2 ? 'sm:border-l sm:border-[var(--line)]' : '',
                i === 4 ? 'lg:border-l lg:border-[var(--line)]' : '',
              ].filter(Boolean).join(' ')}>
              <span className="w-11 h-11 rounded-lg flex items-center justify-center text-2xl grayscale-[.15] group-hover:grayscale-0 transition-all duration-400 group-hover:scale-110"
                style={{ background: `${div.color}14`, boxShadow: `inset 0 0 0 1px ${div.color}33` }}>
                {div.icon}
              </span>
              <div>
                <span className={`${display.className} block text-lg text-[#2a2418] group-hover:text-[#14110c] leading-tight transition-colors`}>{div.label}</span>
      ...
      {/* ── 6. WHY EGYPT ────────────────────────────────────────────── */}
      <section className="border-b border-[var(--line)] bg-[#faf8f3] egg-grain relative">
          ...
          {WHY_EGYPT.map((w, i) => (
            <div key={w.label} className={`px-8 sm:px-10 lg:px-14 py-12 border-t border-[var(--line)] ${i % 2 === 1 ? 'sm:border-l' : ''}`}>
              <div className="flex items-baseline gap-3.5 mb-5">
                <span className={`${display.className} text-3xl text-[#14110c]`}>{w.stat}</span>
                <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-[#a67c42]">{w.label}</span>
              </div>
              <p className="text-sm text-[#5f5849] leading-relaxed max-w-md">{w.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {caseStudies.length > 0 && (
        <section className="border-b border-[var(--line)]">
          <div className="flex items-center justify-between px-6 sm:px-10 lg:px-16 pt-12 pb-4">
            <p className="text-[11px] font-mono uppercase tracking-[0.28em] text-[#9c9585]">Recent thinking</p>
            <Link href="/case-studies" className="group inline-flex items-center gap-1.5 text-[11px] font-mono uppercase tracking-[0.18em] text-[#78715f] hover:text-[#a67c42] transition-colors">
              All <span className="transition-transform duration-300 group-hover:translate-x-1">→</span>
            </Link>
          </div>
          <div className="divide-y divide-[var(--line)]">
            {caseStudies.map(cs => (
              <Link key={cs.path} href={cs.path} className="group flex items-center gap-6 px-8 sm:px-10 lg:px-14 py-8 hover:bg-[#faf8f3] transition-colors duration-400">
                {cs.hero_photo_url && (
                  <div className="hidden sm:block shrink-0 w-16 h-16 rounded-lg overflow-hidden opacity-80 group-hover:opacity-100 transition-opacity duration-400 ring-1 ring-[var(--line)]">
                    <Image src={cs.hero_photo_url} alt="" width={64} height={64} className="w-full h-full object-cover" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-mono uppercase tracking-[0.22em] text-[#a67c42] mb-2">Case study</p>
                  <p className={`${display.className} text-lg sm:text-xl text-[#2a2418] group-hover:text-[#14110c] leading-snug transition-colors`}>{cs.title}</p>
                  {cs.description && <p className="text-xs text-[#9c9585] mt-1.5 line-clamp-1 leading-relaxed">{cs.description}</p>}
                </div>
                <span className="shrink-0 text-[#cbb48f] group-hover:text-[#ff6321] transition-all duration-400 group-hover:translate-x-1 text-lg">→</span>
              </Link>
            ))}
          </div>
        </section>
      ...
      {/* ── 8. CTA ──────────────────────────────────────────────────── */}
      <section className="egg-grain egg-vignette relative px-6 sm:px-10 lg:px-16 py-24 sm:py-36 overflow-hidden bg-[#faf8f3] border-b border-[var(--line)]">
        <div className="relative z-10 max-w-3xl">
          <p className="text-[11px] font-mono uppercase tracking-[0.28em] text-[#a67c42] mb-9">Get in touch</p>
          <h2 className={`${display.className} text-[clamp(2.2rem,5vw,4rem)] font-normal leading-[1.02] tracking-tight text-[#14110c] mb-7`}>
            Have a procurement<br /><span className="italic">requirement?</span>
          </h2>
          <p className="text-[#5f5849] text-base sm:text-lg leading-relaxed max-w-xl mb-12">
          ...
          <div className="flex flex-col sm:flex-row gap-8 sm:items-center">
            <Link href="/rfq" className="group relative inline-flex items-center gap-2.5 text-sm font-semibold text-white bg-[#ff6321] px-8 py-4 rounded-full overflow-hidden self-start transition-transform duration-300 hover:scale-[1.03] shadow-[0_8px_30px_-8px_rgba(255,99,33,.5)]">
              <span className="relative z-10">Submit an RFQ</span>
              <span className="relative z-10 transition-transform duration-300 group-hover:translate-x-1">→</span>
              <span className="absolute inset-0 bg-[#14110c] translate-y-full group-hover:translate-y-0 transition-transform duration-400" />
            </Link>
            <div className="text-sm font-mono text-[#78715f] leading-relaxed">
              <a href="mailto:export@egyptglobe.com" className="text-[#4a4437] hover:text-[#ff6321] transition-colors">export@egyptglobe.com</a><br />
              <span className="text-[#a39c8c]">+20 100 772 9844 · WhatsApp available</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── legal trust line ────────────────────────────────────────── */}
      <div className="px-6 sm:px-10 lg:px-16 py-7">
        <p className="text-[10px] font-mono text-[#a39c8c] tracking-[0.14em]">
...
function SectionLabel({ children }) {
  return (
    <div className="flex items-center gap-4 px-6 sm:px-10 lg:px-16 pt-12 pb-4">
      <span className="h-px w-8 bg-[#b8925a]/45" />
      <p className="text-[11px] font-mono uppercase tracking-[0.28em] text-[#9c9585]">{children}</p>
    </div>
  )
}
